...
export const getInternalPluginMetaData = (
	...
	return Object.values(app.internalPlugins.plugins).map((plugin) => {
		...
		const isEnabled =
			app.internalPlugins.config[id as InternalPluginNameType];
		...
		return {
			...
			lastDataModifiedTime:
				typeof plugin.lastSave === "number"
					? new Date(plugin.lastSave)
					: plugin.lastSave,
			author: "Obsidian",
			version: version,
	...
	const pluginData = manifestIds.map((manifestId) => {
		...
		return {
			...
			lastDataModifiedTime: instance
				? typeof instance._lastDataModifiedTime === "number"
					? new Date(instance._lastDataModifiedTime)
					: null
				: undefined,
		};
	});
