...
import { WatchedProxy } from "src/WatchedProxy";
...
export class CommandsWatcher {
    ...
    async _load() {
        ...
        this.commandsLookupWatcher = new WatchedProxy(this.app.commands.commands, 'app.commands.commands', 3);
    ...
    unsubscribeCommands(callback: (commands: Command[]) => void) {
        this.commandsSubscribers = this.commandsSubscribers.filter(c => c !== callback);
    }
