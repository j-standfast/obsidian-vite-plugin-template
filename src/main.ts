...
import { auditCommands, importShortcuts } from "@/audit/importShortcuts";
...
import { TailorCutsDataManager } from "@/data/TailorCutsDataManager";
import { deserializeKeybindings, serializeKeybindings } from "@/utils/serialize";
...
const kmiString = (kmi: KeymapInfo) => "KeymapInfo: " + kmi.modifiers + " + " + kmi.key;
const hkString = (hk: Hotkey) => "Hotkey: " + hk.modifiers.join(",") + " + " + hk.key;
...
export default class TailorCutsPlugin extends Plugin {
	...
	async addPluginsView() {
		const isViewOpen =
			this.app.workspace.getLeavesOfType(VIEW_TYPE_BARRACLOUGH_TAILOR_CUTS_PLUGINS);
